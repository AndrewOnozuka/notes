...
// Main Class File:    PA5Tester.java
// File:               Board.java
// Quarter:            Winter 2022
//
// Author:             Kameron Gano
...
//
// Persons:          William Powers (TA)
//                  - Helped determine need for while loops in move algorithms
//
// Online sources:   N/A
//
//////////////////////////// 80 columns wide //////////////////////////////////
...
/**
 * Contains a selection of methods that check for a defines
 * bard movements
 *
 * Bugs: Move algorithm is inefficient for very large boards due to
 * dependency on while loops based on the length of the board
 *
 * @author Kameron Gano
...
public class Board {
   ...
   /* The probability (times 100) that a randomly generated tile will be a 
   2 (vs a 4) */
   ...
   /* A reference to the Random object, passed in as a parameter in 
   Board's constructors */
    ...
    */
    ...
    public void printBoard() {
        // Iterate through array, print out grid
        // in proper format
    ...
    public void printBoard(String direction) {
        // capture original grid, store inside this.grid
        this.grid = getGrid();
        ...
        for ( int row = 0; row < GRID_SIZE; row++ ) {
            for ( int col = 0; col < grid[row].length; col++ ) {
                originalGrid[row][col] = this.grid[row][col];
            }
        }
        // Move current grid
        this.move(direction);
        ...
        this.printBoard();
        // Set the grid back to the orignal board
    ...
    private boolean canMoveUp() {
        ...
        for ( int i = 0; i < grid.length; i++ ) {
            for ( int j = grid[i].length - 1; j > 0; j-- ) { 
                // If UP is 0 and current spot is non-zero,
                // UPward move possible
                if ( grid[j-1][i] == 0 && grid[j][i] != 0  ) {
                    return true;
                }
                // If current spot and spot UP are non-zero
                // and the same, then an upward move is possible.
    ...
    private boolean canMoveDown() {
        ...
        for ( int i = 0; i < grid.length; i++ ) {
            for ( int j = 0; j < grid[i].length - 1; j++ ) { 
                // If down is 0 and current spot is non-zero,
                // rightward move possible
                if ( grid[j+1][i] == 0 && grid[j][i] != 0 ) {
                    return true;
                }
                // if current spot and spot down are non-zero
                // and the same, then a downward move is possible.
                if ( (grid[j][i] != 0 && grid[j+1][i] != 0) &&
                     (grid[j][i] == grid[j+1][i]) ) {
                    return true;
                }
    ...
    private boolean canMoveRight() {
        ...
        for ( int i = 0; i < grid.length; i++ ) {
            for ( int j = 0; j < grid[i].length - 1; j++ ) { 
                // If spot to the right is 0 and current spot is non-zero,
                // rightward move possible
                if ( grid[i][j+1] == 0 && grid[i][j] != 0 ) {
                    return true;
                }
                // If current spot and spot to the right are non-zero
                // and the same, then a rightward move is possible.
    ...
    private boolean canMoveLeft() {
        ...
        for ( int i = 0; i < grid.length; i++ ) {
            for ( int j = grid[i].length - 1; j > 0; j-- ) { 
                // If spot to the left is 0 and current spot is non-zero,
                // leftward move possible
                if ( grid[i][j-1] == 0 && grid[i][j] != 0 ) {
                    return true;
                }
                // if current spot and spot to the left are non-zero
                // and the same, then a leftward move is possible.
                if ( (grid[i][j] != 0 && grid[i][j-1] != 0) &&
                     (grid[i][j] == grid[i][j-1]) ) {
                    return true;        
                }
     ...
     */
    private void moveUp() {
        // Iterate through array, repeat for loops
        // as many times as it is long
        int repeatIfZeroes = 0;
        while ( repeatIfZeroes <= (this.grid.length - 1) ) {
            for ( int i = 0; i < this.grid.length; i++ ) {
                for ( int j = this.grid[i].length - 1; j > 0; j-- ) {
                    // If next spot is zero, move it over
                    if ( this.grid[j-1][i] == 0 ) {
                        int tempVal = grid[j][i];
                        this.grid[j-1][i] = tempVal;
                        this.grid[j][i] = 0;
                    } 
                }
            }
        ...
        // Sum numbers together if they are next to each other and match
        for ( int i = 0; i < this.grid.length; i++ ) {
            for ( int j = 0; j < this.grid[i].length - 1; j++ ) {
                // Check if numbers are equal
                if ( this.grid[j][i] == this.grid[j+1][i] ) {
                    int sumNums = this.grid[j][i] + this.grid[j+1][i];
                    this.grid[j][i] = sumNums;
                    this.grid[j+1][i] = 0;
                    score += sumNums;
                    continue;
                }
        ...
        // Iterate through array, repeat for loops
        // half as many times as it is long
        int repeatIfZeroesEnd = 0;
        while ( repeatIfZeroesEnd < (this.grid.length / 2) ) {
            for ( int i = 0; i < this.grid.length; i++ ) {
                for ( int j = this.grid[i].length - 1; j > 0; j-- ) {
                    // If next spot is zero, move it over
                    if ( this.grid[j-1][i] == 0 ) {
                        int tempVal = this.grid[j][i];
        ...
        for ( int i = 0; i < this.grid.length; i++ ) {
            for ( int j = 0; j < this.grid[i].length - 1; j++ ) {
                ...
                if ( this.grid[i][j] == this.grid[i][j+1] ) {
                    int sumNums = this.grid[i][j] + this.grid[i][j+1];
                    this.grid[i][j] = sumNums;
                    this.grid[i][j+1] = 0;
                    score += sumNums;
                    continue;
                }
    ...
    private void moveRight() {
        ...
        while ( repeatIfZeroes <= (this.grid.length - 1) ) {
            for ( int i = 0; i < this.grid.length; i++ ) {
                for ( int j = 0; j < this.grid[i].length - 1; j++ ) {
                    // Check for open spaces, switch spaces with zero
                    if ( this.grid[i][j+1] == 0 ) {
                        int tempVal = this.grid[i][j];
                        this.grid[i][j+1] = tempVal;
                        this.grid[i][j] = 0;
                    } 
                }
