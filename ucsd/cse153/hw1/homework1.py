...
def add_delay_effects(audio):
    ...
    # Create output buffer: original audio + 0.5s delay
    output_length = len(audio) + delay_samples
    output = np.zeros(output_length)

    # Mix original (70%) and delayed (30%) signals
    output[:len(audio)] += 0.7 * audio
    output[delay_samples:] += 0.3 * audio
...
def get_lowest_pitch(file_path):
    mid = MidiFile(file_path)
    notes = [msg.note for track in mid.tracks for msg in track 
...
def get_unique_pitch_num(file_path):
    mid = MidiFile(file_path)
    unique_notes = {msg.note for track in mid.tracks for msg in track 
                    if msg.type == 'note_on' and msg.velocity > 0}
...
def featureQ10(file_path):
    # Precompute to avoid recomputation
    lowest = get_lowest_pitch(file_path)
    highest = get_highest_pitch(file_path)
    unique = get_unique_pitch_num(file_path)
    avg = get_average_pitch_value(file_path)
    
    pitch_range = highest - lowest  # New feature

    return [lowest, highest, unique, avg, pitch_range]
